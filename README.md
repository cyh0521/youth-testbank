...
- Word／PDF／直接列印共用頁邊距設定，預設上下左右各 10 mm，可在側邊欄「進階設定 → 試卷版面設定 → 試卷邊距」調整為 5～50 mm；設定儲存在使用者帳號資料。
...
- 試卷版面設定的初始值為系統預設字型、16 px、行距 1.3；行距可調整至 1.0～2.4，表頭固定為 1.6。進階設定的字體與行距只供新試卷使用；各試卷預覽中的調整儲存於該試卷，列印、Word 與 PDF 使用同一份試卷設定。變更預設值前會替尚未保存外觀的既有試卷保留舊值。試卷表頭設定分為「表頭文字」與「欄位位置」兩張卡片，可分別修改預設文字及每個欄位所在列與列內順序，並各自儲存與重設。新試卷建立時會保存當時的表頭；首次變更進階設定前會為尚無表頭快照的既有試卷保存當前設定。之後進階設定的變更只作為新試卷預設值，既有試卷可從預覽中的「試卷表頭」個別修改。預覽、列印、Word 與 PDF 讀取同一份試卷表頭。
- 進階設定中的科目、預設字體與行距、邊距、預設表頭，以及上次選用的紙張大小與欄數皆保存在使用者帳號的後台資料。登入時若帳號尚無對應值，系統會將舊版瀏覽器設定轉移一次，成功後移除舊值；每張試卷自己的外觀與表頭仍保存在該試卷資料中。
...
- 進階設定將班級、姓名、座號三個格數選單緊湊排在「考試範圍」右側；預覽視窗的「試卷表頭」使用相同欄位、範例與配置，變更只作用於目前的試卷或草稿，不覆寫進階設定。年級已從表頭文字與欄位位置移除。
