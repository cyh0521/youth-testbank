...
- 試卷版面設定的初始值為黑體、16 px、行距 1.85；試卷表頭設定分為「表頭文字」與「欄位位置」兩張卡片，可分別修改預設文字及每個欄位所在列與列內順序，並各自儲存與重設。新試卷建立時會保存當時的表頭；首次變更進階設定前會為尚無表頭快照的既有試卷保存當前設定。之後進階設定的變更只作為新試卷預設值，既有試卷可從預覽中的「試卷表頭」個別修改。預覽、列印、Word 與 PDF 讀取同一份試卷表頭。
- 表頭文字以「學年度與學期」合併輸入，科目後可設定考試範圍；學校、學年度與學期、試別、科目與考試範圍的空欄位只顯示淡色範例，不會輸出。科目與考試範圍的範例分別為「例：全民國防教育」、「例：第1-3章」；空白欄位取得焦點時隱藏範例，失焦後恢復。班級、姓名、座號的設定只顯示格數下拉選單，班級欄預設為 6 格；試卷上固定顯示欄名與指定數量的空格，不畫底線，舊設定的格數會自動沿用。試卷表頭第一行的欄位間距為半個全形字寬。
...
- 預覽視窗上方提供「試卷表頭」與「字體與行距」按鈕；字型、字體大小和行距控制在點擊後才展開。試卷表頭僅保留底部線條，預覽、列印與匯出使用相同樣式。
