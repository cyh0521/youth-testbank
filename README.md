...
- Word／PDF 頁邊距共用設定，預設上下左右各 10 mm，可在側邊欄「進階設定 → 版面設定 → 試卷邊距」調整為 5～50 mm；設定儲存在目前瀏覽器。直接列印仍使用上下 18 mm、左右 20 mm 頁邊距。
- 側邊欄「我的帳號」提供個人資料與密碼設定；管理員另有「帳號管理」入口。「進階設定」包含選擇科目、版面設定與表頭設定。
- 版面設定的初始值為黑體、16 px、行距 1.85；表頭設定可調整文字，以及每個欄位所在列與列內順序。預覽、列印、Word 與 PDF 共用這些設定。
...
| `settings.html?view=catalog` | 進階設定：選擇科目、版面（含試卷邊距）與表頭 | ✓ | ✓ |
