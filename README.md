...
- 點選 Word 或 PDF 下載時，會先選擇紙張大小：A4（210×297 mm）、JIS B3（364×515 mm）或 JIS B4（257×364 mm）。選擇會記在目前瀏覽器，並寫入匯出檔案的頁面尺寸。
- Word／PDF 頁邊距共用設定，預設上下左右各 10 mm，可在預覽視窗的「頁邊距」調整為 5～50 mm；設定儲存在目前瀏覽器。直接列印仍使用 A4 與上下 18 mm、左右 20 mm 頁邊距。
- 是非題及選擇題以「（　）題號」作為答題欄；長題目換行時，後續文字對齊第一行題目文字。Word 匯出會明確設定答題欄與題目欄的字型、字級及行距。
...
- 側欄寬度略為縮小。使用者可在「帳號與設定」選擇原始線條頭像或彩色男女預設人像，也可上傳 JPG、PNG、WebP 圖片；上傳圖會縮小後儲存在個人資料中，首頁右上角同步顯示。
- 「帳號與設定」的「密碼設定」分頁會先驗證目前密碼，再更新新密碼。
...
| `settings.html` | 修改個人資料與頭像 | ✓ | ✓ |
| `settings.html` | 密碼設定分頁：驗證目前密碼並設定新密碼 | ✓ | ✓ |
...
5. 在試卷列表中進行預覽、直接列印、Word 或 PDF 匯出、編輯或刪除。下載 Word 或 PDF 時可選擇 A4、JIS B3（364×515 mm）或 JIS B4（257×364 mm）紙張；預覽視窗也可直接下載兩種格式。
