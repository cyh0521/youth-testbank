...
- 表頭文字以「學年度與學期」合併輸入，科目後可設定考試範圍；學校、學年度與學期、試別、科目與考試範圍的空欄位只顯示淡色範例，不會輸出。班級、姓名、座號的設定只顯示格數下拉選單；試卷上固定顯示欄名與指定數量的空格，不畫底線，舊設定的格數會自動沿用。
- 進階設定將班級、姓名、座號三個格數選單緊湊排在「考試範圍」右側；預覽視窗的「試卷表頭」使用相同欄位、範例與配置，兩處共用儲存值。年級已從表頭文字與欄位位置移除。
