...
- 預覽與編輯試卷的「試卷表頭」加入上下兩行的欄位位置拖曳設定；位置區預設收合，文字、格數與位置可一起儲存或取消。編輯視窗的固定按鈕列與內容捲動區分開，避免滾軸延伸到上欄。
- 帳號的表頭欄位位置設定移至「試卷版面設定」第一張卡片，原「試卷表頭設定」分頁及其表頭文字卡片移除；個別試卷的表頭文字仍在預覽或編輯試卷時設定。
- 原「我的帳號」與設定頁整併為側邊欄單一「進階設定」入口，頁內依序為「個人資料」、「常用科目」、「試卷版面設定」；密碼更新併入個人資料。
- 側邊欄在「編碼選題」下方新增「題本列印」入口與預留頁面，列印功能尚未實作。
- 設定頁整併及題本列印入口為目前工作目錄中的變更；正式驗收仍需登入網站確認分頁、導覽與欄位儲存流程。

## 2026-09-25 更新整理

- 試卷表頭欄位預設順序改為第一行「學校、學年度與學期、科目、試別、考試範圍」，第二行「班級、姓名、座號」；預覽與編輯試卷可單獨調整表頭，既有試卷保留自己的設定。
- 表頭第一列在 DOCX 中使用粗體；DOCX 改由 docx.js 產生真正的 OOXML 文件，並依 Word 顯示結果修正字型、行距、格線貼齊、題距、解答標題與分隔線。仍須以實際試卷在 Microsoft Word 中目視驗證。
- 試卷預設題目行距改為 1.3，表頭行距固定 1.6；各試卷獨立保存字體與行距，帳號設定只用於新建試卷。選擇題的題幹、選項與題尾間距改為半形空格。
- 科目偏好、預設版面與表頭、邊距，以及上次使用的紙張大小與單雙欄設定，改存於 Firestore 的使用者帳號資料；舊瀏覽器設定於首次登入時轉移。
- 編輯試卷與預覽畫面的按鈕圖示調整為單色線條；詳細問題、原因和已執行的檢查記錄於 [`fix_report.md`](fix_report.md)。

## 試卷匯出與排版功能（目前狀態）

- Word 匯出與網頁預覽共用試卷內容，表頭、題目、選項、題尾與解答以同一份試卷資料產生；表頭第一列在 Word 中標記為粗體。下載格式為真正的 OOXML `.docx`，由本地附帶的 docx.js 產生。
- 試卷列表和預覽視窗新增 PDF 下載。PDF 由瀏覽器以 `html2pdf.js` 產生，保留預覽外觀；其頁面內容為影像，文字無法像一般文字型 PDF 一樣選取。
- Word／PDF 下載與直接列印前可選擇 A3（297×420 mm）、A4（210×297 mm）或 JIS B4（257×364 mm），以及單欄或雙欄題目版面；兩組選項都有示意圖，上次選擇會隨帳號在不同裝置同步。雙欄時表頭與解答保留整頁寬度。
- Word／PDF／直接列印共用頁邊距設定，預設上下左右各 10 mm，可在「進階設定 → 試卷版面設定 → 試卷邊距」調整為 5～50 mm；設定儲存在使用者帳號資料。
- 側邊欄「進階設定」整合「個人資料」、「常用科目」、「試卷版面設定」三個分頁；密碼更新卡片位於個人資料下方，管理員另有「帳號管理」入口。試卷版面設定依序提供「表頭欄位位置」、「字體與行距」、「試卷邊距」三張圖卡，各自儲存與重設。
- 試卷版面設定的初始值為系統預設字型、16 px、行距 1.3；行距可調整至 1.0～2.4，表頭固定為 1.6。帳號中的字體與行距設定只供新試卷使用；各試卷預覽中的調整儲存於該試卷，列印、Word 與 PDF 使用同一份試卷設定。變更預設值前會替尚未保存外觀的既有試卷保留舊值。「表頭欄位位置」可調整新試卷預設的列與列內順序；表頭文字與格數由預覽或編輯試卷中的「試卷表頭」個別修改。新試卷建立時會保存當時的表頭；首次變更帳號設定前會為尚無表頭快照的既有試卷保存當前設定。預覽、列印、Word 與 PDF 讀取同一份試卷表頭。
- 常用科目、預設字體與行距、邊距、預設表頭，以及上次選用的紙張大小與欄數皆保存在使用者帳號的後台資料。登入時若帳號尚無對應值，系統會將舊版瀏覽器設定轉移一次，成功後移除舊值；每張試卷自己的外觀與表頭仍保存在該試卷資料中。
- 表頭文字以「學年度與學期」合併輸入，科目後可設定考試範圍；學校、學年度與學期、試別、科目與考試範圍的空欄位只顯示淡色範例，不會輸出。科目與考試範圍的範例分別為「例：全民國防教育」、「例：第1-3章」；空白欄位取得焦點時隱藏範例，失焦後恢復。班級、姓名、座號的格數預設為 8、8、0，可選 0–12 格；試卷上固定顯示欄名與指定數量的空格，不畫底線。試卷表頭第一行的欄位間距為半個全形字寬。
- 預覽與編輯試卷的「試卷表頭」提供班級、姓名、座號三個格數選單，以及各欄位文字與位置設定；變更只作用於目前的試卷或草稿，不覆寫帳號設定。年級已從表頭文字與欄位位置移除。
- 「欄位位置」以可拖曳標籤顯示上下兩行，能在同一行排序或跨行移動；儲存後預覽與輸出依新順序排列。既有第三行設定會接到第二行末尾。
- 表頭欄位位置的預設排列：第一行為學校、學年度與學期、科目、試別、考試範圍；第二行為班級、姓名、座號。已儲存的試卷及帳號自訂排列不會因預設值變更而改動。
- 是非題及選擇題以「（　　）題號」作為答題欄；長題目換行時，後續文字對齊第一行題目文字。預覽、列印與 PDF 使用表格定位，Word 匯出改用同一段落的懸掛縮排，避免儲存格交界造成題號後空白與可見編輯記號。
- 試卷預覽預設只顯示題目，可個別勾選「顯示解答」、「顯示解析」與「顯示來源」，畫面立即更新。答案填入各題作答區，來源接在題目末尾，解析接在該題下方；預覽選項不影響列印、Word 或 PDF 內容。
- 預覽視窗上方提供「試卷表頭」與「字體與行距」按鈕，點擊後在頁面內展開設定。修改表頭欄位時，試卷預覽立即更新；表頭設定區提供「儲存變更」、「清除」、「取消」，清除會即時套用空白文字與預設格數，取消會還原本次尚未儲存的調整。編輯試卷視窗也可展開表頭欄位與同底色的即時表頭預覽，表頭可單獨儲存，或由視窗上方的儲存按鈕一併保存。試卷表頭僅保留底部線條，預覽、列印與匯出使用相同樣式。
- 預覽與編輯試卷的表頭設定另有醒目的「調整欄位位置」入口，預設收合；點開後可拖曳欄位於上下兩行排序。拖曳立即更新表頭，並與文字、格數共用「儲存變更」和「取消」。
- 預覽中的答題括號固定寬度，顯示答案時題號與題目不會位移；答案、解析、來源分別以紅色、藍色、綠色標示。
- 預覽中解析首行由左側開始，換行後對齊「【解析】」後的文字；字型選項以「明體」顯示 Noto Serif。
- 試題文字採左右對齊；預覽中的來源標記緊接題尾，不另加空格。
...
- 側欄頂部第一行以白色幼獅 Logo（`img/youth.png`）搭配「幼獅文化」，第二行緊接跨欄白色區塊顯示「線上命題系統」，文字使用側欄底色；整組可點擊，收合後只顯示白獅圖示。側欄使用者資訊已移除；首頁移除問候區，選題入口統一稱為「手動選題」。
- 點擊側欄品牌區可開啟或收合側欄，支援鍵盤操作並記憶狀態。
- 首頁優先呈現建立試卷、題庫概況與最近試卷；管理員另有快速入口。
- 換頁時同步顯示側欄，移除整頁透明淡入與會造成側欄閃爍的跨頁過渡。側欄角色提示只用於顯示，每頁仍需完成 Firebase 驗證才可操作內容。
- 題庫概況整合題型分布長條圖，可依科目與冊別篩選；冊別只顯示課本管理中該科的實際冊別，並合併重複名稱，顯示所選範圍的題數。
- 側欄底部只保留登出操作；首頁右上角以登入者姓名顯示歡迎訊息。
- 側欄寬度略為縮小。使用者可在「進階設定 → 個人資料」選擇原始線條頭像或彩色男女預設人像，也可上傳 JPG、PNG、WebP 圖片；上傳圖會縮小後儲存在個人資料中，首頁右上角同步顯示。
- 「進階設定 → 個人資料」下方的密碼更新卡片會先驗證目前密碼，再更新新密碼。
...
- 試卷列表可依建立時間、班級或科目排序；卡片資訊及操作按鈕使用單色線條圖示。
...
- 試卷輸出：瀏覽器列印、OOXML Word `.docx` 匯出（本地 docx.js）與 PDF 下載（html2pdf.js）
...
├─ manual.html                人工選題
├─ coded.html                 依題目編號選題
├─ booklet.html               題本列印（功能規劃中）
├─ exams.html                 試卷、班級分類、預覽與輸出
├─ settings.html              進階設定與管理員帳號管理（依網址檢視）
...
│  ├─ docx-export.js          OOXML Word 文件組裝
│  └─ icons.js                共用 SVG 圖示
├─ vendor/
│  ├─ docx.iife.js            docx.js 9.6.1 瀏覽器版
│  └─ docx.LICENSE            docx.js MIT 授權
...
首頁科目題數沿用 `settings/questionStats` 的 `bySubject` 統計；科目名稱合併共用預設與課本管理資料，不再查詢各冊題數。
...
5. 在試卷列表中進行預覽、直接列印、Word 或 PDF 匯出、編輯或刪除。列印或下載時可選擇 A3、A4 或 JIS B4 紙張與單欄／雙欄；預覽視窗也提供列印及兩種下載格式。
